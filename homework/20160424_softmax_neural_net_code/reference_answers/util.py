...
def judge(a, b=None):
    ...
    c = np.zeros((m, 1))
    for i in range(0, m):
        max_i, max_j = max_index(np.array([a[i, :]]))
        if b is not None:
            c[i, 0] = b[max_i, max_j]
        else:
            c[i, 0] = max_j
...
def accuracy_rate(y1, y2):
    ...
    num = 0.0
    for i in range(0, m):
        max_i1, max_j1 = max_index(np.array([y1[i, :]]))
        max_i2, max_j2 = max_index(np.array([y2[i, :]]))
        if max_i1 == max_i2 and max_j1 == max_j2:
            num += 1
    return num / m
...
def load_mnist(url, is_image=True):
    ...
    with open(url, "rb") as f:
        ...
        if is_image:
            ...
            data = np.where(np.array(data) > 100, 1, 0)  # 图片像素值处理，可根据自己需要调整
