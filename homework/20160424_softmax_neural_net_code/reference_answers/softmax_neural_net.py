...
class Layer(object):
    def __init__(self, num, next_num, lamb=0.1, alpha=0.35, epsilon=1.0, soft_max=False):
        self.num = num + 1
        self.lamb = lamb
        self.alpha = alpha
    ...
    def forward_propagation(self):
        if self.soft_max:
            return util.soft_max(self.a, self.theta)
        else:
            return util.sigmoid(self.a, self.theta)

    def back_propagation(self, delta):
        if self.theta is None:
            self.delta = delta
        elif self.soft_max:
            self.delta = np.dot(delta, self.theta.T)[:, 1:]
        else:
...
class NeuralNetworks(object):
    def __init__(self, input_num, output_num, hidden_num=None, lamb=0.1, alpha=0.3, epsilon=1.0, times=200):
        ...
        if len(hidden_num) == 0:
            self.layer = [Layer(input_num, output_num, lamb, alpha, epsilon, soft_max=True)]
            # next_num = 0
            ...
            self.layer.append(Layer(hidden_num[length - 1], output_num, lamb, alpha, epsilon, soft_max=True))
            # next_num = 0
            self.layer.append(Layer(output_num, 0, lamb, alpha, epsilon))
    ...
    def cost_function(self, y):
        m, _ = np.shape(y)
        return -1 / m * np.sum(y * np.log(self.layer[self.layer_num - 1].a[:, 1:]))
...
def main():
    ...
    train_x_url = "train-images.idx3-ubyte"
    train_y_url = "train-labels.idx1-ubyte"
    test_x_url = "t10k-images.idx3-ubyte"
    test_y_url = "t10k-labels.idx1-ubyte"
    x = util.load_mnist(train_x_url)
    y = util.load_mnist(train_y_url, is_image=False)
    x, y = util.get_classify(x, y, 3)
    ...
    x = util.load_mnist(test_x_url)
    y = util.load_mnist(test_y_url, is_image=False)
    x, y = util.get_classify(x, y, 3)
    print "正确率：%.2f%%" % (util.accuracy_rate(y, neural_networks.predict(x)) * 100)
    last_time = time.time()
