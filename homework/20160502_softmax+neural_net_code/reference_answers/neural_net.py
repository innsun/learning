...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = -np.sum(np.log(a2[range(N), y])) / N + reg * np.sum(W1 ** 2) / 2 + reg * np.sum(W2 ** 2) / 2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    delta2 = a2.copy()
    delta2[range(N), y] -= 1
    grads['W2'] = np.dot(a1.T, delta2) / N + reg * W2
    grads['b2'] = np.sum(delta2, axis=0) / N
    delta = z1.copy()
    delta[delta > 0] = 1
    delta[delta < 0] = 0
    delta1 = np.dot(delta2, W2.T) * delta
    grads['W1'] = np.dot(X.T, delta1) / N + reg * W1
    grads['b1'] = np.sum(delta1, axis=0) / N
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      # index = np.argsort(np.random.rand(1, num_train)[0, :])[0:batch_size]
      # X_batch = X[index]
      # y_batch = y[index]
  ...
  def predict(self, X):
    ...
    X_flat = np.dot(np.maximum(0, np.dot(X, W1)), W2)
    exp_scores = np.exp(X_flat - np.max(X_flat))
    pros = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
    y_pred = np.argmax(pros, axis=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
