...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    a2 = z2 + 0
    a2[a2 < 0] = 0
    h = np.exp(np.dot(a2, W2) + b2)
    scores = h / np.array([h.sum(axis=1)]).T
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    y = types_change(y, y.max()+1)
    loss = -(np.log(scores) * y).sum() / len(y) + (reg * W1 * W1 / 2).sum() + (reg * W2 * W2 / 2).sum()
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    v1 = np.dot(y - scores, W2.T)
    d = z2 + 0
    d[d > 0] = 1
    d[d <= 0] = 0
    delta2 = -v1 * d
    grads['W2'] = -np.dot(a2.T, y - scores) / len(y) + reg * W2
    grads['b2'] = -(y - scores).mean(axis=0)
    grads['W1'] = np.dot(X.T, delta2) / len(y) + reg * W1
    grads['b1'] = delta2.mean(axis=0)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      X_batch = X[index[0:batch_size]]
      y_batch = y[index[0:batch_size]]
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
