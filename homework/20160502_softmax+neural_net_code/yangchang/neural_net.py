...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -(np.log(h[np.arange(N), y])).sum() / N + reg * 0.5 * (np.sum(W1 * W1) + np.sum(W2 * W2))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        deta = np.copy(h)
        deta[np.arange(N), y] -= 1
        grads['W2'] = np.dot(a2.T, deta) / N + reg * W2
        grads['b2'] = np.sum(deta, axis=0) / N
        d_w1 = -np.dot(deta, W2.T) * np.where(a2 > 0, 1, 0)
        grads['W1'] = -np.dot(X.T, d_w1) / N + reg * W1
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in xrange(num_iters):
            ...
            #########################################################################
            N = X.shape[0]
            indexs = np.random.choice(N,batch_size)
            X_batch = X[indexs]
            y_batch = y[indexs]
            #########################################################################
            #                             END OF YOUR CODE                          #
