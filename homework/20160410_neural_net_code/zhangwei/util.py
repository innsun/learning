...
class public_util:
    ...
    # 读取数据
    def load_sentiment_data(self, url):
        data = pd.read_csv(url)
        x = data.iloc[:, 1:]
        y = data.iloc[:, 0]
        return x.values, y.values

    def makeconf(self, conf_arr, model, dataSet):
    ...
    # 输出数据
    def print_data(self, time_cost, test_y, y, file_type, file_name):
        ...
        self.makeconf(conf_arr, file_name, file_type)
        print '总时间:', '%.2f' % time_cost, '秒'
